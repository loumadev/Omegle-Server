...
class OmegleClient extends EventListener {
	...
	connect(settings = {lang: "sk"}, count = 0) {
		if(count > 2 && this.isConnected) {
			this.dispatchEvent(Event.LISTENING_ERROR);
			return false;
		}
		...
		fetch(`https://front14.omegle.com/start?caps=recaptcha2&firstevents=1&spid=&randid=${this.randID}${str}`, this.getOptions("json", "")).then(res => res.json()).then(json => {
			...
			this.socket = new WebSocket("wss://front14.omegle.com/wsevents?id=" + encodeURIComponent(this.clientID));
			this.socket.onerror = e => {
				if(!this.name) return;
				Out.error("socket.onerror > reconnecting in 2000ms", this.name, e.reason);
				setTimeout(() => {
					if(this.isConnected) this.connect(settings, count + 1);
				}, 2000);
			};
			...
			this.socket.onmessage = e => {
				...
				if(data === null) {
					if(this.isConnected) this.dispatchEvent(Event.STRANGER_DISCONNECT);
				} else {
