...
new Command("register", "reg").on("execute", e => {
 ...
 > Pri ďalšej návšteve sa prihlásiš pomocou "/login <meno> <heslo>".
===========`, "announcement")
...
new Command("nick").on("execute", e => {
    ...
    let nick = e.args.join(" ").trim().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\s/g, "_").replace(/[^\w]/g, "");
    ...
    if(executor instanceof Client) {
        ...
        if(executor.isLogged) {
            ...
            if(!Auth.updateAccount(executor.name, account)) client.send(`[Príkaz] Vyskytla sa chyba pri ukladaní nicku!`);
        }
...
new Command("suffix").on("execute", e => {
    let client = e.executor;
    let suffix = " " + e.args.join(" ").trim().replace(/\s/g, "_");

    if(!client.hasPermission("account.self.suffix")) return client.send(`[Príkaz] Na tento príkaz nemáš práva!`);
    if(!client.isLogged) return client.send(`[Príkaz] Nie si prihlásený!`);
    if(!suffix.match(/^\w{0,16}$/)) return client.send(`[Príkaz] Suffix musí mať 3-16 znakov a môže obsahovať a-z, 0-9 a _`);

    var account = Auth.getAccount(client.name);
    account.suffix = suffix;
    client.displayName = account.prefix + account.name + account.suffix;

    if(!Auth.updateAccount(client.name, account)) client.send(`[Príkaz] Vyskytla sa chyba pri ukladaní suffixu!`);
    else client.send(`[Účet] Tvoj suffix bol nastavený na "${suffix}"!`);
});
...
new ShopItem("Suffix", 50, "Text za menom").on("buy", e => {
    let client = e.client;
    let account = Auth.getAccount(client.name);

    account.permissions.push("account.self.suffix");
    if(!Auth.updateAccount(client.name, account)) client.send(`[Shop] Vyskytla sa chyba pri ukladaní povolenia!`);

    e.messages = [`Pre zmenu suffixu napíš "/suffix <novy_suffix>"!`, `Suffix si od teraz môžeš meniť bez obmezdení!`];
});
